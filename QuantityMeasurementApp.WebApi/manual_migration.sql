...
-- Delete broken sessions first
IF OBJECT_ID('UserSessions', 'U') IS NOT NULL AND OBJECT_ID('Users', 'U') IS NOT NULL
    EXEC('DELETE FROM UserSessions WHERE UserId IN (SELECT Id FROM Users WHERE PasswordHash = '''' OR PasswordHash IS NULL)');
GO

-- Delete broken users
IF OBJECT_ID('Users', 'U') IS NOT NULL
    EXEC('DELETE FROM Users WHERE PasswordHash = '''' OR PasswordHash IS NULL');
GO

-- Verify
IF OBJECT_ID('Users', 'U') IS NOT NULL
    EXEC('SELECT Id, Email, PasswordHash FROM Users');
GO
