...
IF OBJECT_ID('QuantityMeasurements', 'U') IS NOT NULL
    EXEC('SELECT Id, Operation, FirstValue, FirstUnit, SecondValue, SecondUnit, Result, CreatedAt FROM QuantityMeasurements ORDER BY Id');
ELSE
    PRINT 'Table QuantityMeasurements does not exist yet.';
GO
...
ELSE
    PRINT 'Table does not exist yet - skipping IDENTITY check.';
GO
...
WHERE tbl.name = 'QuantityMeasurements'
